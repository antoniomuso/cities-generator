...
- **add_multi_nodes_or**: it takes a node `A` and a set of nodes `Bn` (with relative costants)and creates the following rule A := .. | B | B<sub>1</sub> | ... | B<sub>n</sub>. If a rule for `A` exists, it merges the old rule with the new.
- **add_multi_nodes_and**: it takes a node `A` and a set of nodes `Bn` and creates the following rule A := .. | B<sub>1</sub> and B<sub>2</sub> and ... B<sub>n</sub>
